...
run_step() {
    ...
    while true; do
        ...
        if [ "$rc" -eq 0 ]; then
            ...
            printf "\r\033[K \033[1;31m✘\033[0m  %s (Failed with code %d)\n" "$msg" "$rc"
            show_step_error "$cmd" "$rc"
            
            echo -e "\n\033[1;33mStep failed. Choose an option:\033[0m"
            echo -e "  \033[1;36m[r]\033[0m Retry this step"
            echo -e "  \033[1;36m[s]\033[0m Skip this step and continue anyway"
            echo -e "  \033[1;36m[d]\033[0m Open an interactive debug shell"
            echo -e "  \033[1;36m[a]\033[0m Abort installation"
            
            local choice
            read -p "  Enter choice [r/s/d/a]: " choice
            choice=$(echo "$choice" | tr '[:upper:]' '[:lower:]')
            
            case "$choice" in
                r|retry)
                    echo -e "\033[33mRetrying: $msg...\033[0m"
                    continue
                    ;;
                s|skip)
                    echo -e "\033[33mSkipping: $msg...\033[0m"
                    return 0
                    ;;
                d|debug)
                    echo -e "\033[33mEntering debug shell. Type 'exit' to return to installer.\033[0m"
                    echo -e "\033[32mHint: Command that failed was: $cmd\033[0m"
                    echo -e "\033[32mLog file: $INSTALL_LOG\033[0m"
                    bash -i
                    continue
                    ;;
                a|abort|*)
...
show_step_error() {
    ...
    echo -e "\033[1;33mCommand failed:\033[0m $failed_cmd"
    echo -e "\033[1;33mExit code:\033[0m $exit_code"
    echo -e "\033[1;33mLast log lines:\033[0m"
    if [ -f "$INSTALL_LOG" ]; then
        tail -n 35 "$INSTALL_LOG"
    else
...
function self_update_script() {
    ...
    local URL="https://raw.githubusercontent.com/callmedimi/MirBot/main/install.sh"
...
function diagnose_bot() {
    clear
    banner
    _sec "Bot Diagnostics & Status"
    
    # Check services
    if systemctl is-active --quiet apache2; then
        _kv "Apache" "$(_dot ok) ${C_OK}Running${CR}"
    else
        _kv "Apache" "$(_dot bad) ${C_BAD}Stopped${CR}"
    fi

    if systemctl is-active --quiet mysql; then
        _kv "MySQL" "$(_dot ok) ${C_OK}Running${CR}"
    else
        _kv "MySQL" "$(_dot bad) ${C_BAD}Stopped${CR}"
    fi

    # Check Database Connection
    if [ -f "$CONFIG_FILE_DEFAULT" ]; then
        local db_h; db_h=$(grep -E '^\$dbhost\s*=' "$CONFIG_FILE_DEFAULT" | cut -d"'" -f2 | cut -d'"' -f2)
        local db_n; db_n=$(grep -E '^\$dbname\s*=' "$CONFIG_FILE_DEFAULT" | cut -d"'" -f2 | cut -d'"' -f2)
        local db_u; db_u=$(grep -E '^\$usernamedb\s*=' "$CONFIG_FILE_DEFAULT" | cut -d"'" -f2 | cut -d'"' -f2)
        local db_p; db_p=$(grep -E '^\$passworddb\s*=' "$CONFIG_FILE_DEFAULT" | cut -d"'" -f2 | cut -d'"' -f2)
        
        # Check mysql connection
        if mysql -h"$db_h" -u"$db_u" -p"$db_p" -e "use $db_n; SELECT 1" >/dev/null 2>&1; then
            _kv "Database" "$(_dot ok) ${C_OK}Connected successfully${CR}"
        else
            _kv "Database" "$(_dot bad) ${C_BAD}Connection failed${CR}"
        fi
        
        # Check webhook status via Telegram API
        local bot_tok; bot_tok=$(grep -E '^\$APIKEY\s*=' "$CONFIG_FILE_DEFAULT" | cut -d"'" -f2 | cut -d'"' -f2)
        if [ -n "$bot_tok" ]; then
            local webhook_info; webhook_info=$(curl -s --max-time 5 "https://api.telegram.org/bot${bot_tok}/getWebhookInfo" 2>/dev/null)
            if echo "$webhook_info" | grep -q '"ok":true'; then
                local wh_url; wh_url=$(echo "$webhook_info" | sed -E 's/.*"url":"([^"]*)".*/\1/')
                local pending; pending=$(echo "$webhook_info" | sed -E 's/.*"pending_update_count":([0-9]*).*/\1/')
                local err_msg; err_msg=$(echo "$webhook_info" | sed -E 's/.*"last_error_message":"([^"]*)".*/\1/')
                
                _kv "Webhook URL" "${C_KEY}${wh_url:-None}${CR}"
                _kv "Pending Msgs" "${C_KEY}${pending:-0}${CR}"
                if echo "$webhook_info" | grep -q "last_error_message"; then
                    _kv "Webhook Error" "${C_BAD}${err_msg}${CR}"
                else
                    _kv "Webhook SSL" "$(_dot ok) ${C_OK}Healthy${CR}"
                fi
            else
                _kv "Telegram API" "$(_dot bad) ${C_BAD}Cannot reach Telegram or invalid token${CR}"
            fi
        fi
    else
        _kv "Config" "$(_dot bad) ${C_BAD}Bot not installed or config.php missing${CR}"
    fi
...
function manage_services() {
    while true; do
        clear
        banner
        _sec "Service Management & Logs"
        _mi "1" "Restart Apache (Web Server)"
        _mi "2" "Restart MySQL (Database)"
        _mi "3" "View Apache Error Logs"
        _mi "4" "View Bot Installation Logs"
        _mi "5" "Back to Main Menu"
        _rule
        echo ""
        printf  "  ${C_PROMPT}❯${CR} Select an option ${C_DIM}[1-5]${CR}: "
        read -r choice
        case "$choice" in
            1)
                echo -e "\033[33mRestarting Apache...\033[0m"
                sudo systemctl restart apache2
                echo -e "\033[32mApache restarted.\033[0m"
                sleep 1.5
                ;;
            2)
                echo -e "\033[33mRestarting MySQL...\033[0m"
                sudo systemctl restart mysql
                echo -e "\033[32mMySQL restarted.\033[0m"
                sleep 1.5
                ;;
            3)
                clear
                banner
                _sec "Apache Error Logs (Last 50 lines)"
                tail -n 50 /var/log/apache2/error.log 2>/dev/null || echo "No logs found."
                echo ""
                printf "  ${C_PROMPT}❯${CR} Press Enter to return... "
                read -r _
                ;;
            4)
                clear
                banner
                _sec "Mirza Installation Logs"
                tail -n 50 /tmp/mirza_install.log 2>/dev/null || echo "No logs found."
                echo ""
                printf "  ${C_PROMPT}❯${CR} Press Enter to return... "
                read -r _
                ;;
...
function backup_restore_db() {
    if [ ! -f "$CONFIG_FILE_DEFAULT" ]; then
        clear
        banner
        _sec "Database Backup & Restore"
        echo -e "  ${C_BAD}●${CR} ${C_BAD}Mirza is not installed. Database operations are unavailable.${CR}"
        echo ""
        printf "  ${C_PROMPT}❯${CR} Press Enter to return... "
        read -r _
        show_menu
    ...
    while true; do
        ...
        banner
        _sec "Database Backup & Restore"
        _mi "1" "Create Backup (SQL file)"
        _mi "2" "Restore Backup"
        _mi "3" "Back to Main Menu"
        _rule
        echo ""
        printf  "  ${C_PROMPT}❯${CR} Select an option ${C_DIM}[1-3]${CR}: "
        read -r choice
        case "$choice" in
            1)
                local backup_file="/root/mirza_db_backup_$(date +%Y%m%d_%H%M%S).sql"
                echo -e "\033[33mCreating database backup...\033[0m"
                if mysqldump -h"$db_h" -u"$db_u" -p"$db_p" "$db_n" > "$backup_file" 2>/dev/null; then
                    echo -e "\033[32mBackup successfully created: $backup_file\033[0m"
                else
                    echo -e "\033[31mFailed to create database backup!\033[0m"
                fi
                sleep 3
                ;;
            2)
                ...
                echo "Available SQL backups in /root:"
                local backups; backups=( $(find /root -maxdepth 1 -name "mirza_db_backup_*.sql" 2>/dev/null) )
                ...
                printf "\n  Select a backup to restore [1-$((idx-1))] (or press enter to cancel): "
                read -r b_idx
                if [ -n "$b_idx" ] && [ "$b_idx" -ge 1 ] && [ "$b_idx" -lt "$idx" ] 2>/dev/null; then
                    local target_backup="${backups[$((b_idx-1))]}"
                    echo -e "\033[31;1mWARNING: This will overwrite the current database!\033[0m"
                    printf "Are you sure? (y/n): "
                    read -r confirm
                    if [[ "$confirm" =~ ^[Yy]$ ]]; then
                        echo -e "\033[33mRestoring database...\033[0m"
                        if mysql -h"$db_h" -u"$db_u" -p"$db_p" "$db_n" < "$target_backup" 2>/dev/null; then
                            echo -e "\033[32mDatabase restored successfully from $(basename "$target_backup")!\033[0m"
                        else
                            echo -e "\033[31mFailed to restore database!\033[0m"
                        fi
                        sleep 3
                    fi
                fi
...
function show_menu() {
    ...
    _mi "5" "Bot Diagnostics & Status"
    _mi "6" "Manage Services & Logs"
    _mi "7" "Database Backup & Restore"
...
function show_help_screen() {
    ...
    _kv "--email" "${C_DIM}Email for Let's Encrypt notifications${CR}"
...
write_apache_config() {
    ...
    # Ensure ports.conf is set to 8008 and 88
    echo -e "Listen 8008\nListen 88" | sudo tee /etc/apache2/ports.conf > /dev/null

    local vhost_file="/etc/apache2/sites-available/${domain}.conf"
    cat <<EOF > "$vhost_file"
# تنظیمات اختصاصی پورت 88 (برای Webhook تلگرام و امنیت بالا)
<VirtualHost *:88>
    ServerName ${domain}
...
<VirtualHost *:8008>
    ...
    RewriteRule ^ https://%{SERVER_NAME}:88%{REQUEST_URI} [END,NE,R=permanent]
</VirtualHost>
EOF
    ...
    sudo rm -f /etc/apache2/sites-enabled/000-default.conf /etc/apache2/sites-enabled/default-ssl.conf
}
...
configure_ssl() {
    ...
    echo -e "[*] Configuring SSL for ${domain} without port conflicts..."

    # 1. Ensure the webroot directory exists for Certbot validation
    mkdir -p "${bot_dir}/.well-known/acme-challenge"
    chown -R www-data:www-data "$bot_dir"

    # 2. Request certificate via webroot (Prevents Certbot from touching Apache ports)
    if sudo certbot certonly --webroot -w "$bot_dir" \
        -d "$domain" \
        --email "$email" \
        --agree-tos \
        --no-eff-email \
        ...
        echo -e "[✓] SSL Certificate successfully acquired/verified!"

        # 3. Write Apache config
        write_apache_config "$domain" "$bot_dir"

        if apache2ctl configtest 2>/dev/null | grep -q "Syntax OK"; then
            echo -e "[✓] Apache configuration syntax is correct. Restarting..."
            sudo systemctl restart apache2
        else
            echo -e "[ERROR] Apache configuration syntax failed. Force clearing ports..."
            sudo fuser -k 8008/tcp 88/tcp >/dev/null 2>&1
            sudo systemctl restart apache2
        fi
    else
        echo -e "[ERROR] Certbot failed to validate SSL."
        return 1
    fi
}
...
preflight() {
    ...
    local b8008 b88
    b8008=$(ss -ltnH 'sport = :8008' 2>/dev/null | head -1)
    b88=$(ss -ltnH 'sport = :88' 2>/dev/null | head -1)
    if [ -n "$b8008" ] || [ -n "$b88" ]; then
        _kv "Ports 8008/88" "$(_dot warn) ${C_WARN}in use (will be freed for Apache/SSL)${CR}"
    else
        _kv "Ports 8008/88" "$(_dot ok) ${C_OK}free${CR}"
    fi
...
function install_bot() {
    ...
    if [ -f "$CONFIG_FILE_DEFAULT" ] || [ -d "$BOT_DIR_DEFAULT" ]; then
        ...
        if [ -f "$CONFIG_FILE_DEFAULT" ]; then
            _sec "Existing Installation Details"
            
            # Extract variables from config.php safely
            local db_h; db_h=$(grep -E '^\$dbhost\s*=' "$CONFIG_FILE_DEFAULT" | cut -d"'" -f2 | cut -d'"' -f2)
            local db_n; db_n=$(grep -E '^\$dbname\s*=' "$CONFIG_FILE_DEFAULT" | cut -d"'" -f2 | cut -d'"' -f2)
            local db_u; db_u=$(grep -E '^\$usernamedb\s*=' "$CONFIG_FILE_DEFAULT" | cut -d"'" -f2 | cut -d'"' -f2)
            local db_p; db_p=$(grep -E '^\$passworddb\s*=' "$CONFIG_FILE_DEFAULT" | cut -d"'" -f2 | cut -d'"' -f2)
            local bot_tok; bot_tok=$(grep -E '^\$APIKEY\s*=' "$CONFIG_FILE_DEFAULT" | cut -d"'" -f2 | cut -d'"' -f2)
            local adm_id; adm_id=$(grep -E '^\$adminnumber\s*=' "$CONFIG_FILE_DEFAULT" | cut -d"'" -f2 | cut -d'"' -f2)
            local dom; dom=$(grep -E '^\$domainhosts\s*=' "$CONFIG_FILE_DEFAULT" | cut -d"'" -f2 | cut -d'"' -f2)
            local bot_usr; bot_usr=$(grep -E '^\$usernamebot\s*=' "$CONFIG_FILE_DEFAULT" | cut -d"'" -f2 | cut -d'"' -f2)
            
            # Fallbacks if cut -d"'" did not match (in case double quotes were used)
            [ -z "$db_h" ] && db_h=$(grep -E '^\$dbhost\s*=' "$CONFIG_FILE_DEFAULT" | cut -d'"' -f2)
            [ -z "$db_n" ] && db_n=$(grep -E '^\$dbname\s*=' "$CONFIG_FILE_DEFAULT" | cut -d'"' -f2)
            [ -z "$db_u" ] && db_u=$(grep -E '^\$usernamedb\s*=' "$CONFIG_FILE_DEFAULT" | cut -d'"' -f2)
            [ -z "$db_p" ] && db_p=$(grep -E '^\$passworddb\s*=' "$CONFIG_FILE_DEFAULT" | cut -d'"' -f2)
            [ -z "$bot_tok" ] && bot_tok=$(grep -E '^\$APIKEY\s*=' "$CONFIG_FILE_DEFAULT" | cut -d'"' -f2)
            [ -z "$adm_id" ] && adm_id=$(grep -E '^\$adminnumber\s*=' "$CONFIG_FILE_DEFAULT" | cut -d'"' -f2)
            [ -z "$dom" ] && dom=$(grep -E '^\$domainhosts\s*=' "$CONFIG_FILE_DEFAULT" | cut -d'"' -f2)
            [ -z "$bot_usr" ] && bot_usr=$(grep -E '^\$usernamebot\s*=' "$CONFIG_FILE_DEFAULT" | cut -d'"' -f2)

            _kv "Bot Username" "${C_KEY}@${bot_usr:-Unknown}${CR}"
            _kv "Bot Token" "${C_KEY}${bot_tok:-Unknown}${CR}"
            _kv "Admin Chat ID" "${C_KEY}${adm_id:-Unknown}${CR}"
            _kv "Domain/Host" "${C_KEY}${dom:-Unknown}${CR}"
            _kv "DB Name" "${C_KEY}${db_n:-Unknown}${CR}"
            _kv "DB User" "${C_KEY}${db_u:-Unknown}${CR}"
            _kv "DB Pass" "${C_KEY}${db_p:-Unknown}${CR}"
            _kv "DB Host" "${C_KEY}${db_h:-Unknown}${CR}"
            echo ""
        fi
...
EOF
    ...
    run_step "Opening firewall ports 80, 443, 8008, & 88" "ufw allow 80 && ufw allow 443 && ufw allow 8008 && ufw allow 88" \
        || { show_step_error; echo -e "\e[91mError: Failed to open firewall ports.\033[0m"; exit 1; }
    run_step "Installing Let's Encrypt (certbot)" "apt-get install -y certbot && systemctl enable certbot.timer" \
        || { show_step_error; echo -e "\e[91mError: Failed to install certbot.\033[0m"; exit 1; }

    # Ensure ports.conf is set to 8008 and 88
    echo -e "Listen 8008\nListen 88" | sudo tee /etc/apache2/ports.conf > /dev/null

    # Write a temporary VirtualHost on port 8008 for webroot challenge verification
    local temp_vhost="/etc/apache2/sites-available/${DOMAIN_NAME}.conf"
...
EOF
    ...
    run_step "Starting Apache on custom port 8008" "systemctl enable apache2 && systemctl restart apache2" \
        || { show_step_error; echo -e "\e[91mError: Failed to start Apache2.\033[0m"; exit 1; }

    if [ -n "$ARG_EMAIL" ]; then
        ssl_email="$ARG_EMAIL"
        echo -e "  ${C_DIM}Email (from --email):${CR} ${C_KEY}${ssl_email}${CR}"
    else
        read -p "Enter email for Let's Encrypt notifications (e.g. admin@$DOMAIN_NAME): " ssl_email
        [ -z "$ssl_email" ] && ssl_email="admin@$DOMAIN_NAME"
    fi

    if ! configure_ssl "$DOMAIN_NAME" "$ssl_email" "$BOT_DIR"; then
        echo -e "\e[91mError: SSL Configuration failed.\033[0m"
        exit 1
    fi
...
\$domainhosts = '${YOUR_DOMAIN}:88';
\$usernamebot = '${YOUR_BOTNAME}';
?>
EOF
            sleep 1
            run_step "Setting Telegram webhook" \
                "curl -s -F \"url=https://${YOUR_DOMAIN}:88/index.php\" -F \"secret_token=${secrettoken}\" \"https://api.telegram.org/bot${YOUR_BOT_TOKEN}/setWebhook\"" \
            ...
            run_step "Starting Apache" "systemctl start apache2" \
                || { show_step_error; echo -e "\e[91mError: Failed to start Apache2.\033[0m"; exit 1; }
            sleep 5
            url="https://${YOUR_DOMAIN}:88/table.php"
...
function update_bot() {
    ...
    if [ -f "$CONFIG_PATH" ]; then
        if ! grep -q "domainhosts.*:88" "$CONFIG_PATH"; then
            echo -e "\e[33mUpdating \$domainhosts in config.php to use port 88...\033[0m"
            local old_dom; old_dom=$(grep "^\$domainhosts" "$CONFIG_PATH" | cut -d"'" -f2 | cut -d'/' -f1)
            if [ -n "$old_dom" ]; then
                local pure_dom; pure_dom=$(echo "$old_dom" | cut -d':' -f1)
                sed -i "s|\\$domainhosts = '${old_dom}';|\\$domainhosts = '${pure_dom}:88';|g" "$CONFIG_PATH"
            fi
        fi
    fi
    ...
    if [ -n "$DOMAIN_NAME" ]; then
        ...
        write_apache_config "$DOMAIN_NAME" "$BOT_DIR"
...
function remove_bot() {
    ...
    echo -e "\e[33mResetting firewall rules...\033[0m" | tee -a "$LOG_FILE"
    sudo ufw delete allow 8008/tcp 2>/dev/null || true
...
function migrate_to_pro() {
    ...
    rm -rf "$OLD_BOT_DIR"
    mkdir -p "$NEW_BOT_DIR"
    ZIP_URL="https://github.com/callmedimi/MirBot/archive/refs/heads/main.zip"
...
ARG_NAME=""     ARG_TOKEN=""   ARG_ADMIN=""    ARG_DOMAIN=""   ARG_EMAIL=""
